...
class SortRow extends React.Component {
    constructor(props) {
        super(props);
        const { layout } = props.list.state.active;
...
class SortableListView extends React.Component {
 ...
 this.setState({
    ...
    renderRow = (data, section, index, highlightfn, active) => {
        ...
        return (
          <Component
            ...
            onRowActive={
                this.handleRowActive
            }
            onRowLayout={
                this._updateLayoutMap(index)
            }
          />
        );
    }
    ...
    renderActive = () => {
        if (!this.state.active) return;
        const { index } = this.state.active.rowData;
